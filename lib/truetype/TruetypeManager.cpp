...
void truetypeClass::end()
{
  ...
  if (table) {
    free(table);
    table = NULL;
    Serial.println("delete table---->");
  }
  this->freePointsAll();
  if (cmapEncoding) {
    delete cmapEncoding;
    cmapEncoding = NULL;
    Serial.println("delete cmapEncoding---->");
  }
  
  if (points) {
    free(points);
    points = NULL;
    Serial.println("delete points---->");
  }
  
  if (pointsToFill) {
    free(pointsToFill);
    pointsToFill = NULL;
    Serial.println("delete pointsToFill---->");
  }
  
  if (beginPoints) {
    free(beginPoints);
    beginPoints = NULL;
    Serial.println("delete beginPoints---->");
  }
  if (endPoints) {
    free(endPoints);
    endPoints = NULL;
    Serial.println("delete endPoints---->");
  }
  
  if (userFrameBuffer) {
    free(userFrameBuffer);
    userFrameBuffer = NULL;
  }
  
  if (glyph.endPtsOfContours) {
    free(glyph.endPtsOfContours);
    glyph.endPtsOfContours = NULL;
    Serial.println("delete glyph.endPtsOfContours1---->");
  }
  
  if (glyph.points) {
    free(glyph.points);
    glyph.points = NULL;
    Serial.println("delete glyph.points---->");
  }
}
...
uint8_t truetypeClass::readSimpleGlyph(uint8_t _addGlyph) {
  ...
  if (_addGlyph) {
    uint16_t* new_endPtsOfContours = (uint16_t *)realloc(glyph.endPtsOfContours, (sizeof(uint16_t) * glyph.numberOfContours));
    if (new_endPtsOfContours == NULL) {
        // 处理内存分配失败
        this->end();
        return 0; // 返回错误
    }
    glyph.endPtsOfContours = new_endPtsOfContours;
  } else {
...
uint8_t truetypeClass::readGlyph(uint16_t _code, uint8_t _justSize) //justSize 仅限大小
{
  ...
  //Serial.println("glyph.numberOfContours:"+String(glyph.numberOfContours));
...
void truetypeClass::textDraw(int16_t _x, int16_t _y, const char _character[]) {
  ...
  this->textDraw(_x, _y, wcharacter); // 调用wchar_t版本的textDraw
  free(wcharacter); // 在return前释放内存
}
...
uint16_t truetypeClass::getStringWidth(const char _character[]) {
  ...
  uint16_t width = this->getStringWidth(wcharacter);
  free(wcharacter); // 在return前释放内存
  return width;
}
...
uint16_t truetypeClass::getStringWidth(const String _string) {
  ...
  this->stringToWchar(_string, wcharacter);
  uint16_t width = this->getStringWidth(wcharacter);
  free(wcharacter);  // 必须在return前释放内存
  return width;
}
...
{
  ttCoordinate_t* new_points = (ttCoordinate_t*)realloc(this->points, sizeof(ttCoordinate_t) * (this->numPoints + 1));
  if (new_points == NULL) {
    // 处理内存分配失败
    this->end();
    return;
  ...
  this->points[this->numPoints].x = _x;
  this->points[this->numPoints].y = _y;
  this->numPoints++;
}
//添加起点
void truetypeClass::addBeginPoint(uint16_t _bp) {
  uint16_t* new_beginPoints = (uint16_t *)realloc(this->beginPoints, sizeof(uint16_t) * (this->numBeginPoints + 1));
  if (new_beginPoints == NULL) {
    // 处理内存分配失败
    this->end();
    return;
  }
  this->beginPoints = new_beginPoints;
  this->beginPoints[this->numBeginPoints] = _bp;
  this->numBeginPoints++;
}

void truetypeClass::addEndPoint(uint16_t _ep) {
  uint16_t* new_endPoints = (uint16_t *)realloc(this->endPoints, sizeof(uint16_t) * (this->numEndPoints + 1));
  ...
  this->endPoints[this->numEndPoints] = _ep;
  this->numEndPoints++;
}
