...
uint8_t truetypeClass::readGlyph(uint16_t _code, uint8_t _justSize)//justSize 仅限大小
{
    ...
    //Serial.printf("    getGlyphOffset 用时: %d ms\n", millis() - start);
    start = millis();
    file.seek(offset);
    //Serial.printf("    file.seek 用时: %d ms\n", millis() - start);
  ...
  Serial.print("glyph.yMax:");Serial.println(glyph.yMax);*/
    ...
    if (glyph.numberOfContours >= 0) {
        ...
        return this->readCompoundGlyph();//读取复合字形
...
void truetypeClass::textDraw(int16_t _x, int16_t _y, const wchar_t _character[]) {
        ...
        /*if ((_character[c] > 0 && _character[c] <= 32) || (_character[c] == L'　')) // 处理ASCII码
    {
      ...
      continue; // 跳过
        ...
        //Serial.printf("  readGlyph 时间: %d ms\n", millis() - start);
        ...
        if (prev_code != 0 && this->kerningOn) {
            ...
            //Serial.printf("  getKerning 用时: %d ms\n", millis() - start);
        ...
        //Serial.printf("  getHMetric 用时: %d ms\n", millis() - start);
        ...
        /*if (_character[c] == '\n')
    {
      ...
      continue;
        ...
        if (glyph.numberOfContours >= 0) {
            ...
            //Serial.printf("  generateOutline 用时: %d ms\n", millis() - start);
            // 填充容器
            start = millis();
            this->fillGlyph(hMetric.leftSideBearing + _x, _y, width);
            //Serial.printf("  fillGlyph 用时: %d ms\n", millis() - start);
            // Serial.println();
        }
        this->freePointsAll();
        this->freeGlyph();
